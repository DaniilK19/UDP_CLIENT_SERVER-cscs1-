...
public class ProgramTests
{
    ...
    [TestMethod]
    public void TestSendGetWidthCommand()
    {
        // Arrange
        var udpClient = new UdpClient();
        var serverEndPoint = new IPEndPoint(IPAddress.Parse("127.0.0.1"), 12345);
        var testWidth = 800; // «м≥н≥ть на реальну ширину
        var expectedCommand = "get width: ";

        using (var sw = new StringWriter())
        {
            Console.SetOut(sw);
            Console.SetIn(new StringReader($"{testWidth}\n"));

            // Act
            Program.SendGetWidthCommand(udpClient, serverEndPoint);

            // Assert
            var result = sw.ToString().Trim();
            Assert.AreEqual($"¬вед≥ть шир≥ну:  оманда над≥слана на сервер: {expectedCommand}", result);
        }
    }

    [TestMethod]
    public void TestSendGetHeightCommand()
    {
        // Arrange
        var udpClient = new UdpClient();
        var serverEndPoint = new IPEndPoint(IPAddress.Parse("127.0.0.1"), 12345);
        var testHeight = 600; // «м≥н≥ть на реальну висоту
        var expectedCommand = "get height: " ;

        using (var sw = new StringWriter())
        {
            Console.SetOut(sw);
            Console.SetIn(new StringReader($"{testHeight}\n"));

            // Act
            Program.SendGetHeightCommand(udpClient, serverEndPoint);

            // Assert
            var result = sw.ToString().Trim();
            Assert.AreEqual($"¬вед≥ть висоту:  оманда над≥слана на сервер: {expectedCommand}", result);
        }
    }
